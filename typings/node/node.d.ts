 ...
 * A Buffer is similar to an array of integers but corresponds to a raw memory allocation outside the V8 heap.  A
 * Buffer cannot be resized. Valid string encodings: 'ascii'|'utf8'|'utf16le'|'ucs2'(alias of
 * 'utf16le')|'base64'|'binary'(deprecated)|'hex'
 */
declare var Buffer: {
  /**
   ...
   * Valid string encodings in Node 0.12: 'ascii'|'utf8'|'utf16le'|'ucs2'(alias of
   * 'utf16le')|'base64'|'binary'(deprecated)|'hex'
   *
   ...
   *   If totalLength is not provided, it is read from the buffers in the list. However, this adds an additional loop
   *   to the function, so it is faster to provide the length explicitly.
   */
  concat(list: Buffer[], totalLength?: number): Buffer;
  /**
...
declare module "http" {
  ...
  export interface AgentOptions {
    ...
    maxSockets?: number;
    /**
     * Maximum number of sockets to leave open in a free state. Only relevant if keepAlive is set to true. Default =
     * 256.
     */
...
declare module "path" {
   ...
   * When multiple slashes are found, they're replaced by a single one; when the path contains a trailing slash, it is
   * preserved. On Windows backslashes are used.
   *
   ...
   * actually the reverse transform of path.resolve.
   *
   * @param from
   * @param to
   */
   ...
   * If there is no '.' in the last portion of the path or the first character of it is '.', then it returns an empty
   * string
   *
   * @param p the path to evaluate.
   */
