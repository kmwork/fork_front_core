...
### !!! Breaking changes:
1. В конфиге изменен адрес для авторизационного сервера для синхронизации с ```@kmwork/auth-server@2.1.0``` (порт теперь 1338, протокол: https и игнорирование неподписанных сертификатов)
было
```
...
import { initComponents as fcComponentsInitComponents } from '@kmwork/frontCore_Components/lib/common/get-components';
...
```
  "dependencies": {
    "@kmwork/front-core": "file:./coreLibs/front-core",
    "@kmwork/frontCore_Components": "file:./coreLibs/frontCore_Components"
  },
```
...
### !!! Breaking changes:
1. Настроил ServiceAuth на работу с сервером ```@kmwork/auth-server@1.0.4```. Добавил Signup \ Forgot password \ Reset password фунционал
...
```
import { getUserAvatarUrl } from '@kmwork/front-core/lib/common/app-redux/reducers/app/users';

<img src={ getUserAvatarUrl(username) } />
```
    
### API Dependencies:
    + ServiceAuth - @kmwork/auth-server@1.1.1
    + ServiceUsers - @kmwork/auth-server@1.1.1
...
### Commits:
    ...
    - feat(extend, test, path): - автоматическое тестирование в дочерних проектах. Достаточно в package.json прописать "cross-env NODE_ENV=test karma start ./node_modules/@kmwork/front-core/test/client/run-client-tests.js"
