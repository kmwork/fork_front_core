...
export function initComponents(COMPONENTS_BASE) {
  ...
  COMPONENTS_BASE.replace('BaseInput', () => ({
    controlRef, touched, isProcessing, value, onTouch, ...props
  }) => <input ref={ controlRef } { ...props } value={ value || '' } />);
  COMPONENTS_BASE.replace('BaseNumberInput', () => ({
    controlRef, touched, isProcessing, value, onTouch, ...props
  ...
  COMPONENTS_BASE.replace('BaseTextArea', () => ({
    controlRef, touched, isProcessing, onTouch, ...props
  }) => <textarea ref={ controlRef } { ...props } />);

  COMPONENTS_BASE.replace('Select', () => require('./components/fields/Select/CoreSelect').default);
  COMPONENTS_BASE.replace('BaseSelect', () => ({ options, value }) => <select value={ value } />);

  COMPONENTS_BASE.replace('DatePicker', () => ({ controlRef, value }) => <input ref={ controlRef } value={ value || '' } type="datetime" />);

  COMPONENTS_BASE.replace('Checkbox', () => ({
    controlRef, touched, isProcessing, onTouch, ...props
